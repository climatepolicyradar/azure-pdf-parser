...
def test_cli(mock_azure_client: AzureApiWrapper, one_page_pdf_bytes: bytes, two_page_pdf_bytes: bytes):
    runner = CliRunner()
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        with patch("src.cli.AzureApiWrapper", return_value=mock_azure_client):        
            result = runner.invoke(cli, ["--pdf-dir", str(pdf_dir), "--output-dir", str(output_dir)])
