...
def test_analyze_document_from_url(
    mock_azure_client: AzureApiWrapper, one_page_mock_analyse_result: AnalyzeResult
) -> None:
...
def test_document_split_one_page(
    ...
    with patch("requests.get") as mock_get:
        ...
        assert page_api_responses[0].extracted_content == one_page_mock_analyse_result
...
def test_document_split_two_page(
    ...
    with patch("requests.get") as mock_get:
        ...
        for i, page_response in enumerate(page_api_responses):
            ...
            assert page_response.extracted_content == one_page_mock_analyse_result
