...
from src.azure_pdf_parser.convert import extract_azure_api_response_paragraphs, extract_azure_api_response_page_metadata
...
def cli(pdf_dir: Path, output_dir: Path):
    ...
    if not AZURE_PROCESSOR_KEY or not AZURE_PROCESSOR_ENDPOINT:
        raise ValueError("Missing Azure API credentials. Set AZURE_PROCESSOR_KEY and AZURE_PROCESSOR_ENDPOINT environment variables.")
    ...
    for pdf_path in tqdm(list(pdf_dir.glob("*.pdf"))):
        ...
        try:
            ...
            LOGGER.error(f"Error processing {pdf_path.name} as short document. Trying again in individual pages.")
            try:
                _, azure_response = azure_client.analyze_large_document_from_bytes(pdf_bytes)
            except:
                LOGGER.error(f"Error processing {pdf_path.name} as individual pages. Skipping.")
                continue
